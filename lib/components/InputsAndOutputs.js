...
const Inputs = ({ inputs }) => {
  return inputs && inputs.map(( input, index ) => {
    ...
    return <div className={'address-value' + inputs.length > 1 && 'mb-2'}>
      <AddressValue
        key={key}
...
const Outputs = ({ outputs }) => {
  return outputs && outputs.map(( output, index )=> {
    return <div
      key={index}
      className={'address-value' + outputs.length > 1 && 'mb-2'}
    >
      <div className={'d-flex justify-content-between'}>
        ...
        >
          <a className={'x-link'} href={`/address/${output.address}`}>{output.address}</a>
        </span>
        <span
          className={clsx(
            'input-amount',
